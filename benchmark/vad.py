...
# Make the project root importable so `vadcore`, `silero`, `webrtc_vad`, and
# `tenvad` can be loaded regardless of where the benchmark is invoked from.
...
def process_directory(
    ...
    **vad_kwargs: Any,
    ...
    """Discover all WAV files under *input_dir* and run the selected VAD on each."""
    ...
    logger.info(
        "VAD params — model=%s  threshold=%.2f  min_silence_ms=%d  padding_ms=%d  sample_rate=%d",
        vad_model, threshold, min_silence_ms, padding_ms, target_sr,
    )
    if vad_kwargs:
    ...
    logger.info("Model loaded (chunk_samples=%d).", vad.chunk_samples)
