...
class SileroStreamingVad:
    def __init__(
        ...
        self._iterator_aggressive: Any = None
    ...
    def _ensure_iterator(self, sample_rate: int) -> None:
        ...
        self._iterator_aggressive = vad_iterator_cls(
            self._model,
            threshold=self.threshold,
            sampling_rate=sample_rate,
            min_silence_duration_ms=self.min_silence_ms,
    ...
    def process_chunk(self, chunk: np.ndarray, sample_rate: int) -> list[VadSegment]:
        self._ensure_iterator(sample_rate)
        ...
        self._stream_offset_s += len(chunk_f32) / float(sample_rate)
        if not event:
            return []
